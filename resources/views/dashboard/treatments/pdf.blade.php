...
<body>
    @php
        $p = $production;
        $gridItems = [];
        $gridItems[] = ['lbl' => 'Resep', 'val' => $p->concept?->name ?? '-'];
        $gridItems[] = ['lbl' => 'Kapasitas', 'val' => formatWeight($p->target_weight_kg).' kg'];
        $gridItems[] = ['lbl' => 'Hari Ke', 'val' => ($p->treatment_day ?? '-').($p->treatment_time ? ' ('.ucfirst($p->treatment_time).')' : '')];
        if ($p->treatment_duration_days) {
            $gridItems[] = ['lbl' => 'Lama Pengobatan', 'val' => $p->treatment_duration_days.' hari'];
        }
        $gridItems[] = ['lbl' => 'Tanggal Campur', 'val' => $p->mix_date?->format('d-m-Y') ?? '-'];
            ...
            <td>
                <div class="card">
                    ...
                    <div class="card-header">
                        {{ $p->name }}
                        <span class="meta">#{{ $p->id }} &middot; Pengobatan</span>
                    </div>
