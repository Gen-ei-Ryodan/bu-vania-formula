...
<head>
    ...
    <style>
        ...
        .info-table td.span-2 { width: 50%; }
        .info-table .treatment-badge {
        ...
        .ttd-box {
            ...
            display: inline-block; min-width: 140px; border-radius: 4px;
        }
...
<body>
    ...
    <div class="report-title">
        <h2>LAPORAN PENGOBATAN</h2>
        <div class="meta">
            #{{ $production->id }} &middot; {{ $production->name }}
            &middot; Dicetak: {{ now()->format('d-m-Y H:i') }} &middot; {{ Auth::user()->name }}
        </div>
    </div>

    @php
        $gridItems = [];
        $gridItems[] = ['lbl' => 'Resep', 'val' => $production->concept?->name ?? '-'];
        $gridItems[] = ['lbl' => 'Kapasitas', 'val' => formatWeight($production->target_weight_kg).' kg'];
        $gridItems[] = ['lbl' => 'Hari Ke', 'val' => ($production->treatment_day ?? '-').($production->treatment_time ? ' &middot; <span class="treatment-badge">'.ucfirst($production->treatment_time).'</span>' : '')];
        if ($production->treatment_duration_days) {
            $gridItems[] = ['lbl' => 'Lama Pengobatan', 'val' => $production->treatment_duration_days.' hari'];
        }
    ...
    <div class="section">
        <div class="section-header">SNAPSHOT ITEM</div>
        <table>
            <thead>
                <tr>
    ...
    @if ($production->groups->isNotEmpty())
        <div class="section">
            <div class="section-header">GOLONGAN</div>
